  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/8 bg-[#06111be6] backdrop-blur-xl">
      {announcementItems.length > 0 ? (
        <div className="border-b border-white/8 bg-gradient-to-r from-orange-400/18 via-sky-400/12 to-lime-300/18">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-4 py-3 sm:px-6 lg:px-8">
            {announcementItems.slice(0, 2).map((announcement) => {
              const toneClass =
              ...
              return (
                <div
                  key={announcement.id}
                  className={`flex flex-col gap-3 rounded-2xl border px-4 py-3 md:flex-row md:items-center md:justify-between ${toneClass}`}
                >
                  ...
                  {announcement.href && announcement.ctaLabel ? (
                    <Link
                      ...
                      className="inline-flex shrink-0 items-center rounded-full border border-current/25 px-4 py-2 text-sm font-medium transition hover:bg-white/10"
                    >
